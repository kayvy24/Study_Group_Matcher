...
def index():
    ...
    if request.method == 'POST':
        ...
        # ✅ Try to join an existing group if possible
                    ...
                    if row['class'] == course:
                        ...
                        if group_times & new_times:
                            ...
                            if len(members) < int(row['group_size']):
                                print(f"✅ APPENDING to existing group {row['group_id']}")
                                members.append(name)
...
"""
    ...
    # Load existing groups to display
    if os.path.exists(GROUPS_FILE):
        with open(GROUPS_FILE, 'r') as gfile:
            reader = csv.DictReader(gfile)
            for row in reader:
...
# --- The rest (manage, update, delete) can remain unchanged unless you'd like updates there too.
